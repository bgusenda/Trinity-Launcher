...
void ImageHandle::SetId(int64_t id) { internal_.id = id; }

int64_t ImageHandle::GetId() const { return internal_.id; }
...
int64_t Activity::GetApplicationId() const { return internal_.application_id; }
...
void FileStat::SetSize(uint64_t size) { internal_.size = size; }

uint64_t FileStat::GetSize() const { return internal_.size; }

void FileStat::SetLastModified(uint64_t lastModified) {
    internal_.last_modified = lastModified;
}

uint64_t FileStat::GetLastModified() const { return internal_.last_modified; }
...
Result LobbyTransaction::SetMetadata(MetadataKey key, MetadataValue value) {
    auto result = internal_->set_metadata(internal_, const_cast<char *>(key),
...
Result LobbySearchQuery::Sort(MetadataKey key, LobbySearchCast cast,
                              MetadataValue value) {
    auto result = internal_->sort(internal_, const_cast<char *>(key),
